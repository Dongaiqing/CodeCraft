...
public interface question_codeDao extends JpaRepository<question_code, Long>{
	@Modifying
	@Query(value = "insert into question_code (source_code,language,user_id,question_id,result) VALUES (:source_code,:language,:user_id,:question_id,:result)", nativeQuery = true)
	@Transactional
	void insertquestion(@Param("source_code") String source_code,@Param("language") String language,@Param("user_id") long user_id,@Param("question_id") long question_id,@Param("result") String result);
